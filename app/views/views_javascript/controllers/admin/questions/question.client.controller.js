(function () {
  ...
  angular.module('questions.admin').controller('QuestionsAdminController', QuestionsAdminController);
  ...
  function QuestionsAdminController($scope, $state, $window, question, Authentication, Notification) {
	...
	vm.addChoice = function(question){
		question.choice.push({})
	}
	  
	vm.removeChoice = function(index){
		vm.question.choice.splice(index,1);
	}
    ...
    function save(isValid) {
      ...
      function successCallback(res) {
        $state.go('admin.questions.list', {questionId: res._id});
